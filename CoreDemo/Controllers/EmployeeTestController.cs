...
namespace CoreDemo.Controllers
{
    public class EmployeeTestController : Controller
    {
        ...
        [HttpGet]
        public IActionResult AssEmployee()
        {
            return View();
        }
        [HttpPost]
        public async Task<IActionResult> AssEmployee(Class1 p)
        {
            var httpClient = new HttpClient();
            var jsonEmployee = JsonConvert.SerializeObject(p);
            StringContent content = new StringContent(jsonEmployee,Encoding.UTF8,"application/json");
            var responseMessage = await httpClient.PostAsync("https://localhost:44357/api/Default",content);
        ...
        public async Task<IActionResult> EditEmployee(Class1 p)
        {
            ...
            var responseMessage=await httpClient.PutAsync("https://localhost:44357/api/Default/" ,content);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
